...
draw_network(edges_prob,title="Site", pal_edges="dodgerblue3", layout="nicely",curv=0.1)$G
...
compar_graphs(compare_output,shade=TRUE)$G
```

However, the user can decide another layout. The nodes position is preserved along the networks.

```{r, fig.height=3.5, fig.width=10}
compar_graphs(compare_output,shade=FALSE, layout="nicely", curv=0.1, base_model="site")$G
```
